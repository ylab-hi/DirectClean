...
class RescueReport:
    ...
    Attributes:
        ...
        details:            Legacy compatibility field.  The normal
                            streaming path writes per-read TSV rows
                            immediately and leaves this list empty.
    """
...
class ReadChopper:
    ...
    def _run_single_thread(self) -> RescueReport:
        ...
        This preserves the existing FASTQ and TSV ordering while avoiding
        accumulation of all output SeqRecords and FinderResult objects.
        """
        report = RescueReport()
            ...
            with open(self.output_fastq, "w") as output_handle:
                for record in read_fastq(self.input_fastq):
                    ...
                    if report_handle is not None:
                        report_handle.write(
                            self._format_report_line(finder_result)
                        )
    ...
    def run(self) -> RescueReport:
        ...
        # The streaming single-thread path writes TSV rows immediately.
        # The legacy parallel path still writes from collected details and
        # will be replaced by bounded ordered streaming in the next commit.
        if self.threads > 1 and self.report_path is not None:
    ...
    @staticmethod
    def _format_report_line(fr: FinderResult) -> str:
        """Format one per-read TSV row exactly as the legacy report."""
    ...
    def _write_report(self, report: RescueReport) -> None:
        ...
        with open(self.report_path, "w") as fh:
            ...
            for fr in report.details:
                fh.write(self._format_report_line(fr))
